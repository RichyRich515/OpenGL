...
sMessage cAnimatedGameObject::message(sMessage const& msg)
{
	if (msg.name == "die")
	{
		if (this->active)
		{
			this->alive = false;
			this->active = false;
			this->skinmesh.forceAnimation("Dying", false, true);
			this->action_timer = this->skinmesh.skinmesh.FindAnimationTotalTime("Dying");
			for (auto robot : robots)
			{
				robot->message(sMessage("stop"));
			}
		}
	}
	return sMessage();
...
{
	for (auto robot : robots)
		if (robot->message(sMessage("dying?")).name == "yes")
			return;
