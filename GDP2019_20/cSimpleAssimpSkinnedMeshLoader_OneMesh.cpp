...
cMesh* cSimpleAssimpSkinnedMesh::CreateMeshObjectFromCurrentModel(unsigned int meshIndex /*=0*/)
{
	...
	pTheMesh->vertices = new cVertex[pTheMesh->numberOfVertices];

	for (int vertIndex = 0; vertIndex != pTheMesh->numberOfVertices; vertIndex++)
	{
		cVertex* pCurVert = &(pTheMesh->vertices[vertIndex]);
	...
	pTheMesh->triangles = new cIndexedTriangle[pTheMesh->numberOfTriangles];

	for (unsigned int triIndex = 0; triIndex != pTheMesh->numberOfTriangles; triIndex++)
	{
		aiFace* pAIFace = &(pAIMesh->mFaces[triIndex]);

		pTheMesh->triangles[triIndex].vert_index_1 = pAIFace->mIndices[0];
		pTheMesh->triangles[triIndex].vert_index_2 = pAIFace->mIndices[1];
		pTheMesh->triangles[triIndex].vert_index_3 = pAIFace->mIndices[2];
